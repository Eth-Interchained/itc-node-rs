...
impl Store {
    ...
    ///
    /// Writes the tip into COLL_HEADERS as "__tip__", then calls
    /// `flush_manifest()` (NEDB v2.5.2+) to atomically snapshot the MANIFEST.
    /// That single fsync is sufficient — warm restart reads the updated MANIFEST
    /// directly, no cold scan needed.
    pub fn put_tip(&self, height: i32, hash: &[u8; 32]) -> io::Result<()> {
        ...
        self.db
            ...
            .map(|_| ())
            .map_err(err)?;
        // Atomic MANIFEST flush — makes tip durable for warm restart (v2.5.2+).
